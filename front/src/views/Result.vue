...
<div class="main-content">
        ...
        <div class="top-info-section">
          ...
          <div class="left-info">
            <!-- 行程概览 -->
            <a-card id="overview" :title="`${tripPlan.start_city || ''} → ${tripPlan.city}旅行计划`" :bordered="false" class="overview-card">
              <div class="overview-content">
                <div class="info-item" v-if="tripPlan.start_city">
                  <span class="info-label">🛫 出发城市:</span>
                  <span class="info-value">{{ tripPlan.start_city }}</span>
                </div>
                <div class="info-item" v-if="userTransportationChoice">
                  <span class="info-label">🚗 出行方式:</span>
                  <span class="info-value">{{ userTransportationChoice }}</span>
                </div>
                <div class="info-item">
            ...
            <a-card id="budget" v-if="tripPlan.budget" title="💰 预算明细" :bordered="false" class="budget-card">
              <div class="budget-grid">
                ...
                <div class="budget-note" v-if="userTransportationChoice">
                  * 交通费用包含往返{{ userTransportationChoice }}费用及其他市内交通费用
                </div>
              </div>
...
import apiClient from '@/services/api' // 添加这行，导入apiClient

const router = useRouter()
const tripPlan = ref<TripPlan | null>(null)
const userTransportationChoice = ref<string>('') // 添加这行
...
onMounted(async () => {
  try {
    // 首先尝试从sessionStorage加载数据
    const data = sessionStorage.getItem('tripPlan')
    const transportationChoice = sessionStorage.getItem('userTransportationChoice')
    
    console.log('从sessionStorage加载的数据:', data)  // 添加此行用于调试
    
    if (data) {
      try {
        tripPlan.value = JSON.parse(data)
        console.log('解析后的tripPlan:', tripPlan.value)  // 添加此行用于调试
        
        // 验证数据结构是否完整
        if (!tripPlan.value || !tripPlan.value.days || tripPlan.value.days.length === 0) {
          console.error('旅行计划数据不完整')
          message.error('旅行计划数据不完整，请重新生成')
          router.push('/')
          return
        }
      } catch (e) {
        console.error('解析tripPlan数据失败:', e)
        message.error('旅行计划数据格式错误，请重新生成')
        router.push('/')
        return
      }
    } else {
      // 如果sessionStorage中没有数据，可能是直接访问了结果页面
      console.error('没有找到旅行计划数据')
      message.warning('没有找到旅行计划数据，请先创建行程')
      router.push('/')
      return
    }
    
    if (transportationChoice) {
      userTransportationChoice.value = transportationChoice
    }
...
const loadAttractionPhotos = async () => {
  ...
  tripPlan.value.days.forEach(day => {
    day.attractions.forEach(attraction => {
      const promise = apiClient.get<{ success: boolean; data: { photo_url: string } }>(`/api/poi/photo?name=${encodeURIComponent(attraction.name)}`)
